  ...
  FileInput.prototype = {
    ...
    _init: function (options, refreshMode) {
      ...
      if (self.errorCloseButton === undefined) {
        self.errorCloseButton = $h.closeButton(
          "kv-error-close" + ($h.isBs(5) ? "  float-end" : "")
        );
      }
      if (
        ...
        setProcessed: function (status) {
          var id = rm.id,
            ...
            skipErrorsAndProceed =
              self.resumableUploadOptions.skipErrorsAndProceed;
          rm.completed = true;
          rm.lastProgress = 0;
          if (hasThumb) {
    ...
    _initTemplateDefaults: function () {
      ...
      //noinspection HtmlUnknownAttribute
      tBtnDefault =
        '<button type="{type}" title="{title}" class="{css}" ' +
      ...
      tActionUpload =
        ...
        "{uploadIcon}</button>";
      tActionRotate =
        '<button type="button" class="kv-file-rotate {rotateClass}" title="{rotateTitle}">' +
        "{rotateIcon}</button>";
      tActionDownload =
    ...
    showUserError: function (msg, params, retainErrorHistory) {
      ...
      if (!params || !params.fileId) {
        ...
        if (!retainErrorHistory) {
          self.$errorContainer
            .find('[data-file-id="' + params.fileId + '"]')
            .remove();
        }
    ...
    _getZoomButton: function (type) {
      var self = this,
        ...
        params =
          title +
          (type === "close"
            ? " data-" + tag + 'dismiss="modal" aria-hidden="true"'
            : "");
      if (
        type === "fullscreen" ||
        type === "borderless" ||
    ...
    _setZoomContent: function ($frame, navigate) {
      var self = this,
        ...
        dir = navigate === "prev" ? "Left" : "Right",
        slideIn = "slideIn" + dir,
        slideOut = "slideOut" + dir,
        parsed,
      ...
      if (zoomData) {
        ...
        parsed = $zoomPreview
          .html()
          .replace(self.zoomPlaceholder, "")
          .setTokens({ zoomData: zoomData });
      ...
      self._handler($modal, "keydown", function (e) {
        ...
        $.each(
          { prev: [$prev, vPrevKey], next: [$next, vNextKey] },
    ...
    _showModal: function ($frame) {
      ...
      $modal.data({
        backdrop: false,
        fileinputPluginId: self.$element.attr("id"),
      });
    ...
    _uploadSingle: function (i, id, isBatch, deferrer) {
      ...
      fnBefore = function (jqXHR) {
        ...
        self._raise("filepreupload", [
          outData,
          previewId,
          i,
          self._getThumbFileId($thumb),
        ]);
      ...
      if (quadrant === 90 || quadrant === 270) {
        ...
        if ($parent.length) {
          ...
          if (hParent > scale * wNew) {
            scale =
              wNew > hParent && wNew != 0 ? (hParent / wNew).toFixed(2) : 1;
          }
        }
        if (scale !== 1) {
          addCss = " scale(" + scale + ")";
    ...
    _getSize: function (bytes, skipTemplate, sizeUnits) {
      ...
      return skipTemplate
        ? out
        : self._getLayoutTemplate("size").replace("{sizeText}", out);
    },
    ...
    _generatePreviewTemplate: function (
      ...
      isRotatable =
        self.fileActionSettings.showRotate &&
        $.inArray(ext, self.rotatableFileExtensions) !== -1;
    ...
    _updateFileDetails: function (numFiles) {
      ...
      log =
        n === 1
          ? label
          : self._getMsgSelected(nFiles, !self.isAjaxUpload && !self.isError);
      if (self.isError) {
    ...
    _isValidSize: function (size, type, $image, $thumb, filename, params) {
      ...
      msg =
        self["msgImage" + type + size] ||
        'Image "{name}" has a size validation error (limit "{size}").';
      self._showFileError(
        msg.setTokens({ name: filename, size: limit, dimension: dim }),
        params
      );
      self._setPreviewError($thumb);
      self.fileManager.remove($thumb);
      ...
      $img
        .one("load", function () {
          ...
          setTimeout(function () {
            ...
            isValidWidth = self._isValidSize(
              "Small",
              "Width",
              $img,
              $thumb,
            ...
            isValidHeight = self._isValidSize(
              "Small",
              "Height",
              $img,
              $thumb,
    ...
    _change: function (e) {
      ...
      if (self.autoReplace) {
        self._getThumbs().each(function () {
          var $thumb = $(this);
          if (
            $thumb.hasClass("file-preview-success") ||
            $thumb.hasClass("file-preview-error")
          ) {
            $thumb.remove();
          }
            ...
            processPreview = function (fType, ext) {
              ...
              if (ext && typeof File !== "undefined") {
                try {
                  ...
                  fileProcessed = new File([file], fName, {
                    type: fileInfo.type,
                  });
      ...
      if (self.uploadAsync) {
        ...
        pool
          ...
          .done(function () {
            ...
            self._raise("filebatchuploadsuccess", [
              fm.stack,
              self._getExtraData(),
            ]);
          })
          .fail(function () {
            ...
            self._raise("filebatchuploaderror", [
              fm.stack,
              self._getExtraData(),
            ]);
          });
  ...
  $.fn.fileinput = function (option) {
    ...
    this.each(function () {
      ...
      var self = $(this),
        ...
        theme =
          options.theme || self.data("theme") || $.fn.fileinput.defaults.theme,
        l = {},
        t = {},
        lang =
          options.language ||
